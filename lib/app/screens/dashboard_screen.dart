...
import '../../core/engine/analytics_engine.dart';
import '../../core/engine/analytics_interfaces.dart';
import '../../core/engine/analytics_models.dart';
import '../../core/engine/feature_module.dart';
import '../../core/engine/knight_engine.dart';
...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  Future<void> _initializeDashboard({bool showLoading = true}) async {
    ...
    recommendationEngine.registerProvider(fitnessModule.recommendationProvider!);
    ...
    final knightContext = knightContextService.buildContext(
      ...
      fitnessSummary: KnightFitnessSummary(
        ...
        capabilityPlaceholder: 'Fitness readiness will update as you log more sessions',
        workoutPlaceholder: 'No workout data yet',
      ),
      travelSummary: KnightTravelSummary(
        visited: travelState.places.where((place) => place.status == TravelPlaceStatus.visited).length,
        wishlist: travelState.places.where((place) => place.status == TravelPlaceStatus.wishlist).length,
        planned: travelState.plannedTrips.length,
        favoritePlaces: travelState.places.where((place) => place.isFavorite).length,
      ...
      setState(() {
        ...
        _scoreUpdatedAt = scores.isEmpty ? DateTime.now() : _latestTimestamp(scores);
        _isLoading = false;
        _isRefreshing = false;
      });
    } catch (_) {
  ...
  Future<List<KnightScoreValue>> _requestScores(ScoringEngine scoringEngine) async {
    if (scoringEngine.providers.isEmpty) {
      return <KnightScoreValue>[];
    }
    return Future.wait(scoringEngine.providers.map((provider) => provider.requestScore()));
  }

  Future<List<KnightAnalyticsSnapshot>> _requestAnalyticsSnapshots(AnalyticsEngine analyticsEngine) async {
  ...
  Future<KnightAnalyticsSnapshot> _buildAnalyticsSnapshot(KnightAnalyticsProvider provider) async {
    ...
    return KnightAnalyticsSnapshot(
      ...
      average: metrics.isEmpty ? 0.0 : metrics.map((metric) => metric.value).reduce((a, b) => a + b) / metrics.length,
  ...
  Future<List<KnightRecommendation>> _requestRecommendations(
    ...
    for (final provider in recommendationEngine.providers) {
      results.addAll(await provider.requestRecommendations(scores: scores, analytics: analyticsSnapshots));
    }
    return results;
  }

  Future<List<KnightMetric>> _requestMetrics(AnalyticsEngine analyticsEngine) async {
  ...
  double _buildOverallScore(List<KnightScoreValue> scores) {
    ...
    return scores.map((score) => score.value).reduce((a, b) => a + b) / scores.length;
  }

  DateTime _latestTimestamp(List<KnightScoreValue> scores) {
    return scores.fold<DateTime>(DateTime.fromMillisecondsSinceEpoch(0), (previous, current) {
      ...
      _engine.featureModules
          ...
          .map(
            (module) => module.searchProvider!.search(
              KnightSearchQuery(moduleId: module.id, text: trimmed, pageSize: 20),
            ),
          ),
    );
  ...
  Widget _buildSearchBar(BuildContext context) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text('Global search', style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.w700)),
        ...
        if (_searchQuery.isNotEmpty) ...[
          ...
          DashboardSection(
            ...
            child: _searchResults.isEmpty
                ? Text('No results found for "$_searchQuery".', style: Theme.of(context).textTheme.bodyMedium)
                : Wrap(
                    spacing: 10,
                    runSpacing: 10,
                    children: _searchResults
                        .map(
                          (result) => Chip(
                            ...
                            avatar: CircleAvatar(child: Text(result.moduleId.substring(0, 1).toUpperCase())),
                          ),
                        )
                        .toList(),
                  ),
  ...
  Widget _buildLearningMentorCard(BuildContext context) {
    ...
    return DashboardSection(
      ...
      subtitle: 'A lightweight coaching loop built from your profile and daily rhythm.',
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('Your mentor is preparing a focused plan from your current goals.', style: Theme.of(context).textTheme.bodyMedium),
          const SizedBox(height: 12),
          ...mentorPlan.map((insight) => Padding(
            ...
            child: Container(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(insight.title, style: Theme.of(context).textTheme.titleSmall?.copyWith(fontWeight: FontWeight.w700)),
          ...
          Text('Next action: capture a voice note or add a reflection so Knight can tailor your next recommendation.', style: Theme.of(context).textTheme.bodyMedium?.copyWith(fontWeight: FontWeight.w600)),
        ],
      ),
    );
  }
  ...
  Widget _buildCompanionWorkspace(BuildContext context) {
    return DashboardSection(
      ...
      subtitle: 'Talk, reflect, or save a private note. Knight will ask before remembering anything.',
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('Use this space for journaling, emotional reflection, or quick thoughts that should stay user-controlled.', style: Theme.of(context).textTheme.bodyMedium),
          const SizedBox(height: 12),
          Wrap(
            spacing: 12,
            runSpacing: 12,
  ...
  Widget _buildHeroScoreCard(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('Knight hub', style: theme.textTheme.titleLarge?.copyWith(fontWeight: FontWeight.w700)),
          const SizedBox(height: 8),
          Text('Central insights from fitness and travel engines.', style: theme.textTheme.bodyLarge?.copyWith(color: theme.colorScheme.onSurfaceVariant)),
          const SizedBox(height: 24),
          Row(
          ...
          Wrap(
            ...
            runSpacing: 12,
            children: [
                  _buildInfoChip(context, 'Modules', '${_knightContext.registeredModules.length}'),
              _buildInfoChip(context, 'Recommendations', '${_knightContext.recommendationSummary.recommendationCount}'),
              _buildInfoChip(context, 'Updated', _formatDateTime(_scoreUpdatedAt)),
        ...
        children: [
          SizedBox(
            ...
            child: DashboardStatCard(
              label: 'Gym profile',
              value: _knightContext.fitnessSummary.gymProfileExists ? _fitnessState.gymProfile.name : 'Not configured',
  ...
  Widget _buildRecommendationsSection(BuildContext context) {
    return DashboardSection(
      ...
      child: _recommendations.isEmpty
          ? Text('No recommendations are available right now.', style: Theme.of(context).textTheme.bodyMedium)
          : Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: _recommendations.take(4).map((recommendation) {
                return Card(
                  ...
                  child: ListTile(
                    leading: CircleAvatar(child: Text(recommendation.category.name.substring(0, 1).toUpperCase())),
      ...
      child: Wrap(
        ...
        children: [
          ...
          SizedBox(
            ...
            child: DashboardStatCard(
              label: 'Top metric',
              value: _analyticsMetrics.isNotEmpty ? _analyticsMetrics.first.name : 'None yet',
  ...
  String _formatDate(DateTime date) {
    final weekdays = <String>['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'];
