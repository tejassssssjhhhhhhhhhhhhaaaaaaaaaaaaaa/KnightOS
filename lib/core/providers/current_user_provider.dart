...
final currentUserProvider = FutureProvider<UserContext>((ref) async {
  ...
  return UserContext(
    ...
    profileSummary: profile == null ? 'New account pending onboarding' : profile.fullName.isEmpty ? 'Profile ready' : profile.fullName,
    preferences: [
      if (profile?.themePreference.isNotEmpty ?? false) profile!.themePreference,
      if (profile?.notificationPreference.isNotEmpty ?? false) profile!.notificationPreference,
    ],
    goals: [
