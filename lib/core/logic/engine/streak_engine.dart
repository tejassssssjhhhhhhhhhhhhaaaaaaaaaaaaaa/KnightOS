class StreakEngine {
  ...
  StreakSummary calculateStreak({
    ...
    for (final date in sortedDates) {
      ...
      longestStreak = longestStreak > streakLength
          ? longestStreak
          : streakLength;
    }
    ...
    for (var index = 0; index < sortedDates.length; index += 1) {
      final date = DateTime(
        sortedDates[index].year,
        sortedDates[index].month,
        sortedDates[index].day,
      );
      if (date.isAtSameMomentAs(lastActive)) {
        ...
        while (nextIndex >= 0) {
          final previousDate = DateTime(
            sortedDates[nextIndex].year,
            sortedDates[nextIndex].month,
            sortedDates[nextIndex].day,
          );
