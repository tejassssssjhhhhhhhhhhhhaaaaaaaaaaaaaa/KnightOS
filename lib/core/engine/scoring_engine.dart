...
/// Main orchestrator for the scoring domain within Knight Engine.
///
/// The scoring engine is intentionally dependency injection friendly and keeps
/// the scoring contracts isolated from feature modules.
class ScoringEngine implements KnightModule {
  /// Creates a scoring engine with optional injected collaborators.
  ScoringEngine({
  ...
  List<KnightScoreProvider> get providers => List<KnightScoreProvider>.unmodifiable(_providers);

  /// Returns the current snapshots as read-only data.
  List<KnightScoreSnapshot> get snapshots => List<KnightScoreSnapshot>.unmodifiable(_snapshots);
  ...
  KnightScoreSnapshot? snapshotFor(KnightScoreCategory category) {
    ...
    return _snapshots.where((snapshot) => snapshot.category == category).cast<KnightScoreSnapshot?>().firstOrNull;
  }
