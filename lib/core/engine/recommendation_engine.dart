...
/// Generic recommendation orchestrator for the Knight Engine platform.
///
/// The recommendation engine is intentionally dependency injection friendly and
/// can support future modules without requiring engine changes.
class RecommendationEngine implements KnightModule, KnightRecommendationHistoryStore {
  /// Creates a recommendation engine with optional collaborators.
  ...
  List<KnightRecommendationProvider> get providers => List<KnightRecommendationProvider>.unmodifiable(_providers);
  ...
  List<KnightRecommendation> prioritizeRecommendations(List<KnightRecommendation> items) {
    // TODO: delegate to the prioritizer if one is registered.
    return items;
  }

  /// Filters the supplied recommendations.
  List<KnightRecommendation> filterRecommendations(List<KnightRecommendation> items) {
