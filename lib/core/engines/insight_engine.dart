...
class InsightEngine {
  ...
  List<String> generateInsights({
    ...
    final averageStress = sessions.isEmpty ? 0.0 : sessions.map((session) => session.stressRating).reduce((value, element) => value + element) / sessions.length;
    final averageFocus = sessions.isEmpty ? 0.0 : sessions.map((session) => session.focusRating).reduce((value, element) => value + element) / sessions.length;
    final profileCompletion = _profileCompletion(profile);

    if (sessions.any((session) => session.shiftType.toLowerCase().contains('night'))) {
