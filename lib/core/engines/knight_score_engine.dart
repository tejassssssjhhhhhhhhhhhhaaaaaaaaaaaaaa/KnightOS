...
class KnightScoreEngine {
  ...
  dynamic calculateScore({
    ...
    if (profile != null || sessions != null) {
      return calculateScoreResult(
        profile: profile ?? const OnboardingProfile(completedSteps: []),
  ...
  int _workProductivityScore(List<WorkSession> sessions) {
    ...
    final ratio = sessions
            .map((session) => session.productiveHours / (session.totalHours == 0 ? 1 : session.totalHours))
  ...
  int _healthGoalScore(OnboardingProfile profile) {
    ...
    if (profile.exerciseFrequency.isNotEmpty || profile.fitnessLevel.isNotEmpty) {
      score += 25;
    }
  ...
  int _focusScore(List<WorkSession> sessions) {
    ...
    final averageFocus = sessions.map((session) => session.focusRating).reduce((value, element) => value + element) / sessions.length;
  ...
  int _energyScore(List<WorkSession> sessions) {
    ...
    final averageEnergy = sessions.map((session) => session.energyRating).reduce((value, element) => value + element) / sessions.length;
