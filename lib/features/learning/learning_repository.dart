...
class LearningRepository {
  LearningRepository({LocalDatabase? localDatabase}) : _database = localDatabase ?? const LocalDatabase();
  ...
  Future<List<LearningGoal>> loadGoals() async {
    ...
    return decoded.whereType<Map<String, Object?>>().map((item) => LearningGoal.fromJson(item)).toList();
  }

  Future<void> saveGoal(LearningGoal goal) async {
    final goals = await loadGoals();
    final updated = <LearningGoal>[goal, ...goals.where((item) => item.id != goal.id)].toList();
